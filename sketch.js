...
function generateSensorNetwork() {
  ...
  while (i <= numberOfSensorNodes) {
    let newSensorNode = new SensorNode(
      random(yellowBox.x, yellowBox.x + yellowBox.width),
      random(yellowBox.y, yellowBox.y + yellowBox.height)
    );
  ...
  for (let i = 0; i < numberOfSensorNodes; i++) {
    for (let j = i + 1; j < numberOfSensorNodes; j++) {
      if (
        sensorNetwork[i].distanceFrom(sensorNetwork[j]) < COMMUNICATION_RANGE
      ) {
...
function isCloseEnough(sensorNetwork, sensorNode) {
  for (let i = 0; i < sensorNetwork.length; i++) {
    let d = dist(
      sensorNetwork[i].position.x,
      sensorNetwork[i].position.y,
      sensorNode.position.x,
      sensorNode.position.y
    );
    if (d < COMMUNICATION_RANGE / 2) return true;
  }
  return false;
}
...
function increaseYellowBoxSize() {
  if (
    yellowBox.x == 0 &&
    yellowBox.y == 0 &&
    yellowBox.width == width / 2 &&
    yellowBox.height == height / 2
  )
    return false;
...
function backgroundGrid() {
  for (let g of grid) g.show();
}

function displaySimulation() {
  if (displayGrid) backgroundGrid();
  population.fittest();
  ...
  if (!population.evolve()) {
    ...
    domainCount++;
    coveredRatio =
      population.networks[population.bestNetworkIndex].areaCovered * 100;
...
function displayParameters() {
  ...
  text(
    "Generations/Population: " + GENERATIONS_PER_POPULATION,
    15,
    (y += offset)
  );
...
function drawLinkGraph() {
  ...
  if (linkHistory.length == GENERATIONS_PER_POPULATION) linkHistory = [];
  linkHistory.push(
    population.networks[population.bestNetworkIndex].findNumberOfLinks()
  );
  stroke(0, 0, 255);
  strokeWeight(1);
  ...
  for (let i = 0; i < linkHistory.length; i++) {
    line(
      ...
      150,
      xoffset + offset,
      150 - (linkHistory[i] / maxValue) * 100
    );
    xoffset += 1;
    offset++;
  }
...
function drawAreaHistoryGraph() {
  ...
  if (domainAreaHistory.length == GENERATIONS_PER_POPULATION)
    domainAreaHistory = [];
  domainAreaHistory.push(
    population.networks[population.bestNetworkIndex].areaCovered
  );
  stroke(0, 0, 255);
  strokeWeight(1);
  ...
  for (let i = 0; i < domainAreaHistory.length; i++) {
    line(
      ...
      320,
      xoffset + offset,
      320 - (domainAreaHistory[i] / maxValue) * 100
    );
    xoffset += 1;
    offset++;
  }
