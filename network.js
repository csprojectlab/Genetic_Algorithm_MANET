...
class Network {
  ...
  deleteSensorNode() {
    ...
    this.sensorNodes = this.sensorNodes.filter(
      (_, index) => index != deleteNodeIndex
    );
    // Deleting the node's links in other nodes....
    this.sensorNodes.forEach(sensorNode =>
      sensorNode.deleteSensorNode(deleteNodeIndex)
    );
  }
  ...
  findCellsCovered() {
    ...
    this.sensorNodes.map(sensorNode => {
      ...
      sensorNode.links.map(linkedNodeIndex => {
        // Cover all it's linked nodes......
        pointB = this.sensorNodes[linkedNodeIndex].position;
        grid.map((cell, index) => {
          // Go through every grid cell....
          if (
            !coveredGrids.includes(index) &&
            collideLineRect(
              pointA.x,
              pointA.y,
              pointB.x,
  ...
  calculateFitness(total_cells) {
    ...
    let fitness =
      pow(this.coveredRatio, 2) -
      this.sensorNodes.length -
      this.findNumberOfLinks();
